...
export default function SubmitReportPage() {
  ...
  // Load all industries and default user industry
  useEffect(() => {
    async function loadInitialData() {
      try {
        ...
        // Fetch all industries for the selector (Demo Mode)
        ...
        const { data: activeInd, error } = await supabase
          .from('industries')
          .select('id, name')
          .eq('user_id', user.id)
          .maybeSingle();
        ...
        if (activeInd) {
          ...
          // Default to the first industry if none is linked to the user (for demo purposes)
          setUserIndustry({ id: industries[0].id, name: industries[0].name });
        }
      } catch (err: any) {
        ...
        <div className="bg-white rounded-2xl shadow-xl p-12 max-w-md w-full text-center">
          ...
          <div className="flex flex-col gap-3 items-center">
            ...
            <div className="flex gap-3 w-full">
              ...
              <Link href="/dashboard/industry" className="flex-1 text-center px-6 py-2.5 bg-[#003366] text-white rounded-lg font-semibold hover:bg-[#004d99] transition-colors flex items-center justify-center">
                Back to Dashboard
              </Link>
            </div>
          </div>
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10">
        ...
        {/* DEMO MODE Selector */}
        <div className="bg-amber-50 border border-amber-200 text-amber-800 px-5 py-3 rounded-xl flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-8 shadow-sm">
          <div className="flex items-center gap-2">
            <span className="bg-[#FF9900] text-white text-[10px] uppercase font-extrabold px-2 py-0.5 rounded">DEMO MODE</span>
            <span className="text-xs font-bold text-amber-800">Reporting for:</span>
          </div>
          <div className="relative min-w-[200px]">
            <select
              value={userIndustry?.id || ''}
              onChange={e => {
                const found = allIndustries.find(ind => ind.id === e.target.value);
                if (found) setUserIndustry({ id: found.id, name: found.name });
              }}
              className="w-full bg-white border border-amber-300 rounded-lg px-3 py-1.5 text-xs font-semibold focus:outline-none focus:ring-1 focus:ring-amber-500 text-amber-900 appearance-none pr-8 cursor-pointer"
            >
